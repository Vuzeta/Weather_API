...
class Weather {
    constructor(city, country, data, wind, humidity, temperature, iconID, weatherDescription) {
        this.city = city;
    ...
    createIcon() {
        const self = this;
        // Elements.icon.setAttribute('src', `http://openweathermap.org/img/w/${this.iconId}.png`);
        Elements.icon.setAttribute('src', `http://openweathermap.org/img/w/${self.iconID}.png`);
    };
    ...
    printTempForFiveDays() {
        ...
        for (let i = 8; i < this.data.length; i += 8) {
            ...
            Elements.tempDaysCome[counterDays].innerHTML = `${Math.round(temp - 273.15)} &deg; C`;
            counterDays++;
        };
    };
    print() {
...
const getWeather = (value) => {
    ...
    req.onload = function () {
        ...
        const newWeather = new Weather(city, country, dateList, wind, humidity, temperature, iconID, weatherDescription);
        newWeather.print();
    };
};
